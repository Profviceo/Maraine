	...
	<div class="wrapper">
		...
		<section class="left">
			...
			<section>
				...
				$update = $_GET['update'];
				if ($update == "submit") {
					echo '<p class="error">An errored Occured</p>';
				}elseif ($update == "invalidchar") {
				...
				}elseif ($update == "invalidstate") {
					echo '<p class="error">You must choose a Nigerian State to continue</p>';
				}elseif($update == "dateinvalid"){
					echo '<p class="error">Your choosen date must be more than 18 years back</p>';
				}elseif($update == "notprepared"){
					echo '<p class="error">An error occured. Please contact our support team</p>';
				}elseif ($update == "donebefore") {
					echo "<p class='error'>You have updated your profile once and you can't update it a second time. Please contact our support team via Support@Maraine.com</p>";
				}elseif ($update == "success"){
					echo '<p class = success>Your profile has been updated successfully</p>';
				}
			}
			?>
			<form action="includes/account.inc.php" method="POST" >
				<h2 style="text-align:center; font-size: 30px; border-bottom: 2px solid black; margin-bottom: 30px;">Profile Details</h2>
				<label>Full- Name</label><br>
				<input type="text" name="fullname" value="<?php echo $_SESSION['name'] ?>" ><br><br>
				...
				<input type="text" name="phone" value="<?php echo $_SESSION['phone'];?>" disabled><br><br>
				<label>Date Of Birth</label><br>
				<input type="date" name="Dob" value="<?php echo $_SESSION['dob'];?>" ><br><br>
				<label>Registration Number</label><br>
				<input type="text" name="regno" value="<?php echo $_SESSION['regno'];?>" disabled><br><br>
				<label>State Of Origin</label><br>
				<input type="text" name="state" value="<?php echo $_SESSION['state']; ?>"><br><br>
				<button type="submit" name="update">Update All</button>
					<div><p>NB: You are only allowed to update your profile  once. After which you <br>will have to contact the support to tell them why you want to change the details and what the error was.<br> Feel free to our support team via the email  <a href="mailto:Support@Maraine.com">Support@Maraine.com.</a></p></div>
			</form>
		</section>
		<section class="profileimg">
			<?php
				...
				if (isset($_GET['imgupdate'])) {
					$imgupdate = $_GET['imgupdate'];

					if ($imgupdate == "submit") {
						echo '<p class="error">An error occured.</p>';
					}elseif ($imgupdate == "nofile") {
						echo '<p class="error">No file was found.</p>';
					}elseif ($imgupdate == "notallowed") {
						echo '<p class="error">This file type is not supported.</p>';
					}elseif ($imgupdate == "filetoobig") {
						echo '<p class="error">This file is to big. You can only upload files with sizes under 5mb</p>';
					}elseif ($imgupdate == "notprepared") {
						echo '<p class="error">An error occured.</p>';
					}elseif ($imgupdate == "success"){
						echo '<p class="success">Your profile image has been updated successfully.</p>';
					}
				}
			?>
			<form action="includes/updateprofileimg.inc.php" method="POST" enctype="multipart/form-data">
				<!-- Creating profile image checker -->
				<?php
					...
					if (!mysqli_stmt_prepare($stmt,$sql)) {
						...
						while ($row = mysqli_fetch_assoc($result)) {
							if ($row['Profile_img_stat'] == 0) {
								echo '<img src="img/profile.png" class="profile_img"><label>Change Profile Image</label><br>';
							}else{
								echo '<img src="img/profile'.$_SESSION['userid'].'.jpg?'.mt_rand().'" class="profile_img"><br><label>Change Profile Image</label><br>';
							}
						}
